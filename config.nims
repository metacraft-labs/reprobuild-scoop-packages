...
## `just`, CI and a bare `nim c` alike, whatever the working directory.
...
## Nothing in this repository passes `--skipParentCfg` today; the Justfile recipes
## pass their own `--nimcache:` under `build/`, which still wins.
